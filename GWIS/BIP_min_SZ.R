...
# cov(X,Y) / Var(X) based on the genetic  covariance between bip and scz, and the genetic variance in SCZ
...
beta <- log(bip_scz$or) - b2 * log(bip_scz$OR)
...
# correlation matrix this is required and based on the gcov int from ldscore regression to guard agains inflation due to sample overlap  (:
cor1 <- matrix(c(1,  0.1018 , 0.1018  , 1),2,2)

for( i in 1:nrow(bip_scz)){
  
  se[i] <- deltamethod(~x1 - b2 * x2, mean=log(as.numeric(bip_scz[i,c(6,19)])),  cov = diag(bip_scz[i,c(7,20)]) %*% cor1 %*% diag(bip_scz[i,c(7,20)]), ses=T )
...
bip_res2 <- cbind(bip_res,bip_scz$FRQ_U_45670,as.numeric(as.character(bip_scz$CEUaf)),(bip_scz$info + bip_scz$INFO)/2, beta,se,p_val)
names(bip_res2)[c(6,7,8)] <- c("EAF"," EAF_bip" ,"INFO")

write.table(bip_res2 ,file="bip_res.txt",row.names=F,quote=F)
